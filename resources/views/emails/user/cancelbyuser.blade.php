...
<head>
    <meta charset="UTF-8">
    <title>Chengivy Store - Hủy đơn thành công!</title>
    <style>
        body {
...
<body>
    <div class="container">
        ...
        <p>Đơn hàng #{{ $orderCancelled->id }} đã được hủy thành công
        </p>
        <p>{{ $orderCancelled->payment_method }}</p>
        <p class="date">[Đơn hàng #{{ $orderCancelled->id }}] ({{ \Carbon\Carbon::parse($orderCancelled->ordered_at)->format('F j, Y') }})</p>
        <p>Ngày nhận hàng dự kiến: <span class="text-primary">{{ \Carbon\Carbon::parse($orderCancelled->estimated_at)->format('F j, Y') }}</span></p>
        <div class="message">
            <table class="table table-bordered">
                ...
                <tbody>
                    <?php $start = 1; ?>
                    @foreach ($orderCancelled->order_product as $item)
                    <tr>
                        <td><?= $start++; ?></td>
                        <td class="d-flex">
                ...
                <tfoot>
                    <tr>
                        ...
                        <th>{{ number_format($orderCancelled->total_price, 0, ',', '.') }}</th>
                    </tr>
                    <tr>
                        <th colspan="5" class="text-bold">Tổng giảm giá</th>
                        <th>{{ number_format($orderCancelled->total_discount, 0, ',', '.') }}</th>
                    </tr>
                    <tr>
                        <th colspan="5" class="text-bold">Phí vận chuyển</th>
                        <th>{{ number_format($orderCancelled->fee, 0, ',', '.') }}</th>
                    </tr>
                    <tr>
                        <th colspan="5" class="text-bold">Tổng đơn đặt hàng</th>
                        <th class="text-primary">{{ number_format($orderCancelled->total_value, 0, ',', '.') }}</th>
                    </tr>
                </tfoot>
            </table>
        </div>
        <div class="delivery">
            <h2>Thông tin số tài khoản</h2>
            <p>Bạn vui lòng truy cập hệ thống và kiểm tra số tài khoản ngân hàng</p>
            <p>Hệ thống sẽ hoàn lại tiền cho bạn trong 2 ngày làm việc</p>
        </div>
        <div class="cta-button">
            <a href="http://localhost:3000/customer/profiles">Truy cập "Hồ sơ tài khoản" tại đây</a>
        </div>
        <div>
